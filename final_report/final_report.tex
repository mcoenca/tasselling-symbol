...
faster than matrix factorization. This may however be due to implementation
kinks that could be smoothed out. 
...
A possible avenue of improvement for this problem would be to investigate
possible combinations of the two approaches that may yield better
results, by drawing on each one's particular strengths.

A second extension would be to extract more features from data we already
...
A third extension would be to do text analysis on reviews in order to extract
critic features (vocabulary, length of reviews, \dots) and movie features (we
could learn the movie genre, what emotions it evokes, \dots). This would
require a significant amount of work, as sentiment analysis is another entire
